...
// This file is in large parts taken from winit (licensed as Apache-2.0).
...
use derive_more::{AsRef, BitAnd, BitAndAssign, BitOr, BitOrAssign, Debug, Not};
...
pub struct KeyEvent {
    ...
    pub repeat: bool,
    /// Modifiers at the time of the event occurring
    pub modifiers: Modifiers,
    // TODO: Consider if we should support this
...
pub enum NativeKeyCode {
    Unidentified,
    /// An Android "scancode".
    Android(#[debug("{_0:#04X}")] u32),
    /// A macOS "scancode".
    MacOS(#[debug("{_0:#04X}")] u16),
    /// A Windows "scancode".
    Windows(#[debug("{_0:#04X}")] u16),
    /// An XKB "keycode".
    Xkb(#[debug("{_0:#04X}")] u32),
}
...
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq, AsRef)]
pub struct Modifiers {
    /// The modifier state
    pub state: ModifierState,
    /// The actual keys pressed.
    ///
    /// Due to platform limitations, this might not contain all keys and [`state`] should be used
    /// when the differenciation between the individual keys is not required.
...
#[must_use]
pub struct ModifierState(#[debug("{_0:b}")] u8);

#[rustfmt::skip]
impl ModifierState {
    /// The "shift" key.
    pub const SHIFT: Self = Self::new(0);
    /// The "control" key.
    pub const CTRL: Self = Self::new(1);
    /// The "alt" key.
    pub const ALT: Self = Self::new(2);
    /// This is the "windows" key on PC and "command" key on Mac.
    pub const SUPER: Self = Self::new(3);
}

impl ModifierState {
    const fn new(n: u8) -> Self {
        assert!(n < 8, "modifier button must be less than 8");
        Self(1 << n)
    }

    #[must_use]
    pub fn has(self, key: Self) -> bool {
        (self & key) == key
    }
...
/// Allows differentiating different locations of the same modifier keys.
///
/// **Note:** Not all platforms allow detecting which key was pressed, therefor it might be that
/// both [`lshift`](Self::lshift) and [`rshift`](Self::rshift) return false, even though
/// [`ModifierState::shift() == true`](ModifierState::shift).
#[derive(
    Clone, Copy, Debug, Default, PartialEq, Eq, BitAnd, BitOr, BitAndAssign, BitOrAssign, Not,
)]
#[must_use]
pub struct ModifierKeys(#[debug("{_0:b}")] u8);

#[rustfmt::skip]
impl ModifierKeys {
    /// The left "shift" key.
    pub const LSHIFT: Self = Self::new(0);
    /// The right "shift" key.
    pub const RSHIFT: Self = Self::new(1);
    /// The left "control" key.
    pub const LCTRL: Self = Self::new(2);
    /// The right "control" key.
    pub const RCTRL: Self = Self::new(3);
    /// The left "alt" key.
    pub const LALT: Self = Self::new(4);
    /// The right "alt" key.
    pub const RALT: Self = Self::new(5);
    /// This is the left "windows" key on PC and "command" key on Mac.
    pub const LSUPER: Self = Self::new(6);
    /// This is the right "windows" key on PC and "command" key on Mac.
    pub const RSUPER: Self = Self::new(7);
}
